...
Run ATP across multiple lexicon sizes for one simulated child.
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run ATP across lexicon sizes for one simulated child."
    )

    parser.add_argument("--root", required=True)
    parser.add_argument("--child_id", "--child", dest="child_id", type=int, required=True)
    ...
    for lexicon_size in args.sizes:
        input_path = os.path.join(args.root, f"child-{args.child_id}", f"{lexicon_size}.txt")
        ...
        tree_out = None
        if args.save_trees:
            tree_out = os.path.join(args.tree_dir, f"child{args.child_id}_{lexicon_size}")
        ...
        print(
            f"Finished child-{args.child_id} size={lexicon_size} | "
    ...
    write_csv(
        os.path.join(args.out_dir, "single_child_summary.csv"),
        summary_rows,
        SUMMARY_FIELDS,
    )
    write_csv(
        os.path.join(args.out_dir, "single_child_rules.csv"),
        rules_rows,
        RULE_FIELDS,
    )
